...
public class CommandBinding<T>(val key: String, val trigger: Observable<Unit>, val handler: CommandHandler<Throwable, T>, val canExecute: Action1<in Boolean> = Action1 {}) {
    ...
    fun bindTo(bindingContext: BindingContext<*>, command: Command<T>) {
        ...
        trigger.flatMap({ v:Unit -> wrapError(command.execute) }).compose(bindingContext.applyLifecycle<T>())
