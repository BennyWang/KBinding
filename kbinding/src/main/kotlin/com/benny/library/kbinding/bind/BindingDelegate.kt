 ...
 */
public class BindingDelegate() : IViewModel {
    ...
    public fun <T> bindProperty(key: String, initialValue: T): ReadWriteProperty<Any?, T> {
        this@BindingDelegate.addProperty(key, Property(initialValue))
        //support for nested view model
        if(initialValue is IViewModel) {
            for((k, v) in initialValue.properties) this@BindingDelegate.addProperty("$key.$k", v)
            for((k, v) in initialValue.commands) this@BindingDelegate.addCommand("$key.$k", v)
        }
    ...
    public fun <T> bindProperty(key: String): ReadWriteProperty<Any?, T?> {
        this@BindingDelegate.addProperty(key, Property<T>())
    ...
    public fun <T> bindCommand(key: String, initialValue: Command<T>): ReadOnlyProperty<Any?, Command<T>> {
        this@BindingDelegate.addCommand(key, initialValue)
